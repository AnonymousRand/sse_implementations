...
- thus, if you want to manually define any one of the Things to override the automatically generated default behavior (which is REQUIRED for all classes with pointer members that own the pointed-to data!!), **you should almost always manually define ALL the Things,** using `= default` for the ones you want to keep the automatically generated behavior of (or `= delete` to disable them entirely). (this along with the destructor is often called the c++ "rule of five".) an explicit default constructor is also often needed along with these, since defining the copy or move constructor prevents a default constructor from being automatically generated as well.

- examples can be found in [src/utils/types/i_disk_storage.h](src/utils/types/i_disk_storage.h), [src/utils/types/db/](src/utils/types/db/), and [src/utils/types/enc_ind/](src/utils/types/enc_ind/) ("the rule of five" methods).
