...
ustring Doc::encode() const {
    ustring ret = utils::str::encodeBigint(this->id, config::INT_MAX_BYTES);
    ret += utils::str::encodeBigint(this->kw, config::INT_MAX_BYTES);
...
Doc Doc::decode(const ustring& encoding) {
    Id id = utils::str::decodeBigint(encoding, 0);
    Kw kw = utils::str::decodeBigint(encoding, config::INT_MAX_BYTES);
...
ustring SrcIDb1Doc::encode() const {
    ustring ret = utils::str::encodeBigint(this->kw, config::INT_MAX_BYTES);
...
SrcIDb1Doc SrcIDb1Doc::decode(const ustring& encoding) {
    Kw kw = utils::str::decodeBigint(encoding, 0);
